...
export function pickReportTemplate(input: { serviceType?: string | null; title?: string | null }): ReportTemplate {
...
const SPECS: Record<ReportTemplate, TemplateSpec> = {
  ...
  gardener: {
    ...
    final: [
      ...
      { key: "waste_bags", label: "Waste bags removed", type: "number" },
      { key: "materials_charges", label: "Charge for materials?", type: "boolean" },
      {
      ...
      { key: "all_tasks_done", label: "All scheduled tasks done?", type: "boolean" },
      { key: "next_visit_tasks", label: "Tasks for next visit", type: "longtext", optional: true },
      { key: "seasonal_maintenance", label: "Seasonal maintenance notes", type: "longtext", optional: true },
    ],
  },
  cleaner: {
    start: [
